...
def get_shop_items():
    ...
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("SELECT item_name, price, description FROM shop ORDER BY price ASC, item_name ASC")
        return cur.fetchall()
    finally:
        cur.close()
        conn.close()
...
def get_shop_item(item_name: str):
    ...
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("SELECT item_name, price, description FROM shop WHERE item_name = %s", (item_name.lower(),))
        return cur.fetchone()
    finally:
        cur.close()
        conn.close()
...
def add_shop_item(item_name: str, price: int, description: str):
    ...
    try:
        cur.execute(
            """
            INSERT INTO shop (item_name, price, description)
            VALUES (%s, %s, %s)
